...
The current compact mobile split has separately passed the live VNC gate across iPhone 14 portrait, iPhone Pro Max portrait, iPhone 14 landscape, and a touch tablet: **147/147 checks** passed with one connected canvas, no horizontal overflow, touch/keyboard input, fullscreen, grid, ratio/zoom adjustment, a deterministic manual iOS-paste fallback, and a visible task chat/composer. The gate also guards against reopening the viewport tools leaving the task workspace hidden. The evidence and remaining physical-iPhone/Safari limits are in [docs/MOBILE-E2E-VALIDATION.md](docs/MOBILE-E2E-VALIDATION.md).
