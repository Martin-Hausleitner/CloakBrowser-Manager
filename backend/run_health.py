...
class HealthSnapshot:
    ...
    def from_dict(cls, data: Mapping[str, Any]) -> HealthSnapshot:
        ...
        return cls(
            ...
            measurement_error=bool(data.get("measurement_error")),
            policy_version=str(data.get("policy_version") or HEALTH_POLICY_VERSION),
            outbound_ip_masked=(
                str(data["outbound_ip_masked"])
                if data.get("outbound_ip_masked") is not None
                else None
            ),
        )
...
def evaluate_health(
    ...
    *,
    ...
    if snapshot.state not in PROCEED_STATES:
        failed.append(snapshot.state)
        return _decision(False, False, failed, active_policy.version)
    ...
    if snapshot.checked_at is None:
        ...
        if age > timedelta(minutes=active_policy.freshness_minutes):
            failed.append("health_stale")
