...
| Phone fit re-tap is idempotent (no update/stop/launch when dims match) | Proven (local branch UI + Manager; prior VCVM E2E retained) | Early-return in `applyProfileViewport`; match-aware status copy; `mobile_ui_gate` re-tap + page mutate counters (update/stop/launch=0) with **counterInstalled** false-zero guard; release requires re-tap name + traffic zeros + installed hooks; durable `phonefit_idempotent_retap_proof.py` uses **same page counters as gate** + Playwright dual witness + release-shaped evidence (19 checks: UI + page/pw traffic + profile updated_at/vnc stable); local re-tap 2026-08-02 Slice H; VCVM PNG 2026-07-31 | Public VCVM deploy of this branch SHA; full multi-viewport gate pack refresh |
